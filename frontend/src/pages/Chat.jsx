...
export default function Chat() {
  ...
  const selectChat = async (chat) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/chats/${chat.id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
            ...
            <div className="space-y-6">
              <div className="aspect-square bg-surface-container rounded-2xl overflow-hidden border border-outline-variant/30 flex items-center justify-center shrink-0">
                {activeChat.product?.images?.[0] ? (
                  <img
                    src={activeChat.product.images[0].startsWith('http') ? activeChat.product.images[0] : `${import.meta.env.VITE_API_URL}${activeChat.product.images[0]}`}
