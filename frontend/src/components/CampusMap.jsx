...
const CampusMap = ({ selectedBus, liveBuses = EMPTY_LIVE, allSchedules = EMPTY_SCHEDS }) => {
  ...
  const busPositions = useMemo(() => {
    ...
    // 1. Fallback all schedule buses to Aryabhatta if they are inactive
    if (Array.isArray(allSchedules)) {
      const uniqueBuses = new Set(allSchedules.map(s => s.bus.busNumber));
      uniqueBuses.forEach((busId, index) => {
        // Add tiny offset based on string so they don't stack perfectly on the same pixel
        const offset = (busId.charCodeAt(busId.length-1) % 7) * 0.0001; 
        positions[busId] = {
           lat: STOP_COORDS['Aryabhatta'].lat + offset,
           lng: STOP_COORDS['Aryabhatta'].lng + offset,
           estimated: true
        };
      });
    }

    // 2. Overwrite with verified live GPS positions if active
