...
export function Map({ children, styles, ...props }) {
  ...
  const resolvedTheme = "dark"; // Forced dark theme as requested
...
export function MarkerPopup({
  ...
  return createPortal(
    <div
      className={cn(
        "relative rounded-md border bg-popover p-3 text-popover-foreground shadow-md animate-in fade-in-0 zoom-in-95 z-50",
        className
      )}
    >
      {closeButton && (
        <button
          type="button"
          onClick={handleClose}
          className="absolute top-1 right-1 z-10 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100"
...
export function MarkerTooltip({
  ...
  return createPortal(
    <div
      className={cn(
        "rounded-md bg-foreground px-2 py-1 text-xs text-background shadow-md animate-in fade-in-0 zoom-in-95 z-50",
        className
      )}
    >
      {children}
