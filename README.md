...
Welcome to the **Campus Connect** repository! This is the central hub for the IIT Patna Super-App. We are currently building **Phase 1: Live Bus Navigation**, featuring real-time telemetry, a modern Soft-UI dashboard, and an interactive daily timetable.
...
* **Frontend:** Runs locally (Vite + React + Tailwind) for maximum speed and hot-reloading.
...
1. **[Docker Desktop](https://www.docker.com/products/docker-desktop/)**: The engine that runs our backend. **Must be open and running in the background before you start coding.**
...
PORT=5000
```
## Step 2: Fire Up the Docker EngineOpen a terminal in the root directory and run the orchestrator.
This will download PostgreSQL and Node.js, install backend dependencies, and link everything together.
```bash
docker compose up -d --build
```
Note: The -d flag runs it in the background. It might take a minute or two the first time.

## Step 3: Initialize the Database (Prisma)Now we need to create the tables in our empty database and inject the official IITP bus schedule.
**Run these commands one by one:Create the tables:**
```bash
...
You should see the Neomorphic dashboard!🧪 How to Test the Live Bus (Simulation)Since we don't have the physical driver app running yet, we use a simulation script to make the bus move on the map.Ensure your Docker containers are running.Open a terminal in the backend folder.
**Run the mock driver:**
```bash
cd backend
node mock-driver.js
```
Watch your frontend map—the bus will start driving its route!🔍 Essential Developer Commands Keep this cheat sheet handy. You will use these commands daily.What you want to do:
